...
# 상속
# 일반유닛
# 만약 메딕이 있다면 데미지는 필요없을 것이다. 따라서 상속을 써주도록 한다.
# AttackUnit가 Unit의 name, hp의 생성자를 상속받아 사용된다.
class Unit:
    def __init__(self, name, hp): # self를 제외한 나머지 인자를 받아주어야함
        self.name = name
        self.hp = hp
...
# 메소드 오버라이딩
class Unit:
    def __init__(self, name, hp, speed): # self를 제외한 나머지 인자를 받아주어야함
        self.name = name
        self.hp = hp
        self.speed = speed
...
# ----- buy 지상 유닛, 공중 유닛 파악이 힘들다. 그래서 우리는 move 함수를 통해 한방에 해결할 것 이다.
vulture.move("11시")
battlecruiser.move("9시") # move함수를 재정의
...
class BuildingUnit(Unit):
    def __init__(self, name, hp, location):
        pass # 함수는 실행됐지만 일단 넘어감

# 서플라이 디폿 : 건물, 1개 건물 = 8, 유닛.
supply_depoy = BuildingUnit("서플라이 디폿", 500, "7시") # 일단 넘어가짐
...
# 드랍쉽
dropship = FlyableUnit() # Unit의 생성자만 생성됨.
# 만약 둘다 출력하고 싶다면 아래로껄로 해야함.
